...
void x86_64_Processor::Init(uint64_t HHDMOffset, MemoryMapEntry** memoryMap, uint64_t memoryMapEntryCount, PagingMode pagingMode, uint64_t kernelVirtual, uint64_t kernelPhysical) {
    ...
    uint64_t kernelStack = (uint64_t)VMM::g_KVMM->AllocateAnonPages(KERNEL_STACK_SIZE / PAGE_SIZE, VMM::DEFAULT_KALLOC_PHYS_FLAGS) + KERNEL_STACK_SIZE;
...
void x86_64_Processor::InitTSS(Scheduler::ProcessorState* state) {
    ...
    m_TSS.IST[0] = (uint64_t)VMM::g_KVMM->AllocateAnonPages(KERNEL_STACK_SIZE / PAGE_SIZE, VMM::DEFAULT_KALLOC_PHYS_FLAGS) + KERNEL_STACK_SIZE;
    m_TSS.IST[1] = (uint64_t)VMM::g_KVMM->AllocateAnonPages(KERNEL_STACK_SIZE / PAGE_SIZE, VMM::DEFAULT_KALLOC_PHYS_FLAGS) + KERNEL_STACK_SIZE;
