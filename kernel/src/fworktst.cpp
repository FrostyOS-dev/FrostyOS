/*
From: https://github.com/iProgramMC/Boron/blob/master/drivers/test/source/fworktst.c
License: BSD 3-Clause (https://github.com/iProgramMC/Boron/blob/master/LICENSE.txt)
*/
...
void Init()
{
	PixBuff = (uint8_t*)g_KFramebuffer.BaseAddress;
	PixWidth = g_KFramebuffer.width;
	PixHeight = g_KFramebuffer.height;
	PixPitch = g_KFramebuffer.pitch;
	PixBPP = g_KFramebuffer.bpp;
...
void* CreateThread(void (*func)(void*), void* data) {
	debug_putc('c');
	Thread* thread = new Thread({func, data}, g_KProcess);
	assert(thread->Init());
	g_KProcess->AddThread(thread);
...
[[noreturn]] void ExitCurrentThread() {
	...
	dbgprintf("exit on %lu\n", state->id);
	Thread::ExitCurrentThread(true);
	PANIC("Thread delete failed");
}
...
{
	PFIREWORK_DATA ParentData = (PFIREWORK_DATA)Parameter;
	FIREWORK_DATA Data;
...
NO_RETURN void T_Explodeable(UNUSED void* Parameter)
{
	...
	{
		PFIREWORK_DATA DataClone = (PFIREWORK_DATA)kcalloc(1, sizeof(FIREWORK_DATA));
		if (!DataClone)
			break;
...
void SpawnParticle(PFIREWORK_DATA Data)
{
	void* Thrd = CreateThread(T_Particle, Data);
	if (!Thrd)
		kfree(Data);
	// else
...
{
	void* Thrd = CreateThread(T_Explodeable, NULL);
	// if (Thrd)
	// 	ObDereferenceObject(Thrd);
}
...
void PerformFireworksTest()
{
	...
	// void* Thrd = CreateThread(CoreUsage, KeGetCurrentThread());
	// if (Thrd)
	// 	ObDereferenceObject(Thrd);
...
// 	void* LookedAtThread = LookedAtThreadV;
...
// 		uint64_t UptimeMS = HAL_GetTicks();
